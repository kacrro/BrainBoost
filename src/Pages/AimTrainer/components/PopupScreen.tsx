...
const PopupScreen: React.FC<PopupScreenProps> = ({
    ...
    useEffect(() => {
        const fetchRecentScores = async () => {
            ...
            if (data) {
                ...
                const alreadyExists = reversed.some(
                    (entry) =>
                        entry.score === averageReactionTime &&
                        entry.created_at &&
                        Math.abs(new Date(entry.created_at).getTime() - new Date(now).getTime()) < 3000
                );

                let allResults: GameResult[] = alreadyExists || averageReactionTime === null
                    ? reversed
                    : [...reversed, { id: newId, score: averageReactionTime, created_at: now }];
        ...
        if (popupVisible && averageReactionTime !== null) {
            fetchRecentScores();
        }
    }, [popupVisible, userEmail, averageReactionTime]);
    ...
    const getPerformanceMessage = (time: number, hits: number) => {
        if (time < 200 && hits >= 8) return "Ekspertowski poziom! 🎯🚀";
        if (time < 250 && hits >= 6) return "Świetny wynik! 🎯👍";
        if (time < 300 && hits >= 5) return "Bardzo dobry wynik! 😊";
        if (time < 400 && hits >= 3) return "Dobry wynik! 🙂";
        if (hits < 3) return "Spróbuj trafić więcej celów! 🎯";
    ...
    return (
        ...
        >
            <div style={popupContentStyle} onClick={(e) => e.stopPropagation()}>
                ...
                {averageReactionTime && (
                    <>
                        <h2>Średni czas reakcji: {averageReactionTime} ms</h2>
                        <p style={{ fontSize: '18px', margin: '16px 0', color: '#333' }}>
                            Trafiłeś {totalHits} celów!
                        </p>
                        <p style={{ fontSize: '18px', margin: '16px 0', color: '#333' }}>
                            {getPerformanceMessage(averageReactionTime, totalHits)}
                        </p>
                        ...
                        {recentScores.length > 1 ? (
                            ...
                            >
                                ...
                                >
                                    Historia czasów reakcji (ostatnie {recentScores.length} gier)
                                </h3>
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <Tooltip
                                            ...
                                            formatter={(value: number) => [`${value} ms`, 'Średni czas reakcji']}
                                        />
                                        <Line
                                            type="monotone"
                                            dataKey="score"
